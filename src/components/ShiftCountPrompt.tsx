...
interface ShiftCountPromptProps {
  initialCount?: number;
  onConfirm: (count: number) => void;
}

export function ShiftCountPrompt({ initialCount = 1, onConfirm }: ShiftCountPromptProps) {
    ...
    <div className="w-full max-w-md mx-auto step-enter">
      <div className="text-center mb-8">
        ...
        <p className="text-muted-foreground">
          Enter how many shifts you worked this period
        </p>
      </div>
      ...
      <div className="rounded-xl border border-border bg-card p-8 shadow-sm">
        <div className="flex items-center justify-center gap-4 mb-6">
          <button
            ...
            aria-label="Decrease shift count"
          >
          ...
          <button
            ...
            className="w-12 h-12 rounded-xl bg-muted flex items-center justify-center
                       hover:bg-muted/80 disabled:opacity-50 disabled:cursor-not-allowed
                       transition-all min-h-[44px] min-w-[44px]"
            aria-label="Increase shift count"
          >
        ...
        <p className="text-center text-sm text-muted-foreground mb-6">
          You&apos;ll log earnings for <strong className="text-foreground">{count}</strong>{' '}
          shift{count !== 1 ? 's' : ''}
        </p>
